...
description: Crear o revisar pruebas unitarias, integración, API, PostgreSQL, concurrencia, conectividad/PWA y E2E para los flujos críticos de Carnavales 2027. Usar para cobertura, regresiones y reproducción de bugs; no para redefinir producto o implementar features ajenas al test.
---
...
- Areas: dominio, API, PostgreSQL, auth, autorización, votos, concurrencia, conectividad, PWA y auditoría.

Prioridad: **critical** pérdida/duplicado/alteración de voto, autorización y corrupción transaccional; **high** conectividad online-first, OTP, cierres, penalizaciones y auditoría; **normal** componentes secundarios, filtros y presentación.
...
## Online-first, PWA y E2E

Flujo online-first prioritario: confirmar voto -> POST directo con `operationUuid` -> éxito bloquea edición; timeout/red no confirma y permite reintento manual; replay idempotente queda confirmado.

También: servidor procesa y pierde respuesta -> cliente reintenta -> backend reconoce idempotencia -> no duplica. Cubrir dependencias/orden, sesión expirada, noche cerrada, conflicto idempotente y reconexiones repetidas.
...
- Pérdida de red: votar -> desconectar -> intentar confirmar -> error claro sin bloqueo -> reconectar -> reintentar manualmente.
...
- Probar selector, modal, bloqueo post-confirmación, cambio de comparsa, conexión, reintento manual y errores API.
