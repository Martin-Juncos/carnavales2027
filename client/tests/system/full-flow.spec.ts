...
test.describe.serial('stack real Carnavales 2027', () => {
  test('Admin → Jurado offline → Fiscal → Escribano', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { name: 'Administración Sistema' })).toBeVisible()
    await expect(page.getByText('#1 · Noche 1', { exact: true })).toBeVisible()
    await expect(page.getByText(/#1 · Ará Berá · noche 1/)).toBeVisible()
    ...
    await expect(page.getByText('Noche 1', { exact: true })).toBeVisible()
    ...
    await page.getByRole('group', { name: /seleccionar nota para diseño/i }).getByRole('button', { name: /nota 4/i }).click()
    await page.getByRole('dialog').getByRole('button', { name: /confirmar nota/i }).click()
    await expect(page.getByText('Nota bloqueada')).toHaveCount(1)
    ...
    await expect(page.getByText('Confirmado por servidor').first()).toBeVisible({ timeout: 20_000 })

    for (const [item, score] of [['Terminación', 5], ['Música', 3]] as const) {
      await page.getByRole('group', { name: new RegExp(`seleccionar nota para ${item}`, 'i') }).getByRole('button', { name: new RegExp(`nota ${score}`, 'i') }).click()
      await page.getByRole('dialog').getByRole('button', { name: /confirmar nota/i }).click()
      await expect(page.getByText('Nota bloqueada')).toHaveCount(score === 5 ? 2 : 3)
      await expect(page.getByText('Confirmado por servidor')).toHaveCount(score === 5 ? 2 : 3, { timeout: 20_000 })
    }
