...
test('juror vote survives lost connectivity, reloads, and syncs idempotently', async ({ page }) => {
  ...
  await page.route('**/api/v1/jurado/contexto', (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: context, meta: {} }) }))
  ...
  await page.getByLabel(/email o dni/i).fill('jurado@example.com')
  await page.getByLabel(/contrase/i).fill('password-segura')
  ...
  await expect(page.getByText('Confirmado por servidor')).toBeVisible({ timeout: 15_000 })
})
